...
export function parsePrompt(input: string): ParseResult {
  ...
  for (const clause of clauses) {
    ...
    // A single clause can name more than one concept (e.g. "wait for a fair
    // value gap during London session" is both a killzone and an fvg) — a
    // single `.find()` here used to keep only the first match and silently
    // drop the rest, with no unmapped-clause warning since the clause DID
    // technically match something. Match every block the clause names.
      ...
      for (const p of matched.params) {
        ...
        if (matched.id === "sweep" && p.key === "of") {
          if (/low/i.test(clause)) {
            params.of = /session/i.test(clause)
              ? "Session Low"
              : "Previous Day Low";
            hadExplicitParam = true;
            continue;
        ...
        if (matched.id === "pdh" && p.key === "side") {
          const hasHigh = /\bhigh\b/i.test(clause);
          const hasLow = /\blow\b/i.test(clause);
          if (hasHigh && !hasLow) {
            params.side = "High";
            hadExplicitParam = true;
            continue;
        ...
        if (
          (matched.id === "risk_per_trade" ||
            matched.id === "max_daily_loss") &&
          p.key === "percent"
        ) {
          const n = extractNumber(clause);
          if (n !== undefined) {
